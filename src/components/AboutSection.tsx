    ...
    <section className="py-24 px-6 bg-white dark:bg-gray-950 overflow-hidden">
      <div className="max-w-6xl mx-auto grid md:grid-cols-2 gap-16 items-center">
        <div ref={ref1 as React.RefObject<HTMLDivElement>} className="reveal">
          ...
          <h2 className="text-4xl md:text-5xl font-bold text-plum-900 dark:text-white mb-6 leading-tight">
            Where Tradition{" "}
            <span className="block italic font-light text-terracotta-600 dark:text-terracotta-400">Meets Innovation</span>
          </h2>
        ...
        <div ref={ref2 as React.RefObject<HTMLDivElement>} className="reveal grid grid-cols-3 gap-6" style={{ transitionDelay: "0.15s" }}>
          {stats.map((s) => (
            <div key={s.label} className="text-center p-6 bg-plum-50/50 dark:bg-gray-900/80 rounded-2xl border border-plum-100 dark:border-gray-800 hover:shadow-md hover:border-terracotta-400/40 transition-all">
              <div className="text-4xl font-bold text-plum-900 dark:text-white mb-2">{s.value}</div>
